...
extern t_redir *redirection_undo_list;
extern t_redir *exec_redirection_undo_list;

static void add_exec_redirect(t_redir *rp);
static int add_undo_redirect(int fd, enum r_instruction r, int fl);
static int add_undo_close_redirect(int f);
static int expandable_redirection_filename(t_redir *rp);
static int stdin_redirection(enum r_instruction r, int fl);
static int undoablefd(int fl);
static int do_redirection_internal(t_redir *rp, int fl);

static int write_here_document(int fl, t_wdtk *wd);
static int write_here_string(int fl, t_wdtk *wd);
static int here_document_to_fd(t_wdtk *wd, enum r_instruction r);

static int redir_special_open(int fd, char *n, int fl, int fll, enum r_instruction r);
static int noclobber_open(char *l, int fl, int fll, enum r_instruction r);
static int redir_open(char *l, int fl, int fll, enum r_instruction r);

static int redir_varassign(t_redir *rp, int fl);
static int redir_varvalue(t_redir *rp);
...
extern void internal_error(const char *fmt, ...); /* error.c */
void	redirection_error(t_redir *temp, int error)
{
	char *filename;
	char *allocname;
	int oflags;
	...
	if ((temp->rflags & REDIR_VARASSIGN) && error < 0)
		filename = allocname = ft_strdup(temp->redirector.filename->word);
	...
	else if (error != NOCLOBBER_REDIRECT && temp->redirector.dest >= 0 && error == EBADF)
	{
		...
		if (temp->ins == r_duplicating_input || temp->ins == r_duplicating_output ||
			temp->ins == r_move_input || temp->ins == r_move_output)
			filename = allocname = ft_itoa(temp->redirectee.dest);
		else if (temp->ins == r_duplicating_input_word)
		{
			if (temp->redirector.dest == 0) /* Guess */
				filename = temp->redirectee.filename->word; /* XXX */
			else
				filename = allocname = ft_itoa(temp->redirector.dest);
		}
		else if (temp->ins == r_duplicating_output_word)
		{
...
int		do_redirections(t_redir *list, int flags)
{
	int		error;
	t_redir	*temp;
...
static int	expandable_redirection_filename(t_redir *redirect)
{
	if (redirect->ins == r_output_direction ||
		redirect->ins == r_appending_to ||
		redirect->ins == r_input_direction ||
		redirect->ins == r_inputa_direction ||
		redirect->ins == r_err_and_out ||
		redirect->ins == r_append_err_and_out ||
		redirect->ins == r_input_output ||
		redirect->ins == r_output_force ||
		redirect->ins == r_duplicating_input_word ||
		redirect->ins == r_duplicating_output_word ||
		redirect->ins == r_move_input_word ||
		redirect->ins == r_move_output_word)
		return (1);
	else
		return (0);
}
...
char	*redirection_expand(t_wdtk *word)
{
	char	*result;
	t_wlst	*tlist1;
	t_wlst	*tlist2;
	...
	old = g_executing_builtin;
	g_executing_builtin = 1;
	sv_ifs("IFS");
	g_executing_builtin = old;
...
static int	write_here_string(int fd, t_wdtk *redirectee)
{
	char	*herestr;
	int		herelen;
	int		n;
...
/* Write the text of the here document pointed to by REDIRECTEE to the file
   descriptor FD, which is alread open to a temp file. Return 0 if the
   write is successful, otherwise return errno. */
static int	write_here_document(int fd, t_wdtk *redirectee)
{
	char			*document;
	int				document_len;
	int				fd2;
	...
	if (tlist)
	{
		...
		while (t)
		{
			...
			document_len = ft_strlen(document);
			if (t != tlist)
				ft_putchar_fd(' ', fd2); /* Separator */
			old = write(fd2, document, document_len);
			if (old < document_len)
			{
				if (errno == 0)
					errno = ENOSPC;
...
static int	here_document_to_fd(t_wdtk *redirectee, enum r_instruction ri)
{
	char	*filename;
	int		r;
	int		fd;
	...
	fd = sh_mktmpfd("sh-thd", MT_USERANDOM|MD_USETMPDIR, &filename);
...
static t_wdtk _redir_special_filenames[] = {
	{"/dev/fd/[0-9]*", RF_DEVFD},
...
static int	redir_special_open(int spec, char *filename, int flags, int mode, enum r_instruction ri)
{
	...
	else if (spec == RF_DEVSTDIN)
		fd = fcntl(0, F_DUPFD, SHELL_FD_BASE);
	else if (spec == RF_DEVSTDOUT)
		fd = fcntl(1, F_DUPFD, SHELL_FD_BASE);
	else if (spec == RF_DEVSTDERR)
		fd = fcntl(2, F_DUPFD, SHELL_FD_BASE);
	else if (spec == RF_DEVTCP || spec == RF_DEVUDP)
	{
		if (g_restricted)
...
/* Do the specific redirection requested. Returns errno or one of the
   ...
   If flags & RX_ACTIVE is zero, the just do whatever is necessary to
   produce the approrpiate side effects. flags & RX_UNDOABLE, if non-zero,
   ...
   close-on-exec. */
static int	do_redirection_internal(t_redir *redirect, int flags)
{
	t_wdtk *redirectee;
	...
	if (TRANSLATE_REDIRECT(ri))
	{
		...
		/* XXX - what to do with [N]<&$w- where w is unsed or NULL? ksh93
		         closes N. */
		...
		else if (all_digits(redirectee_word))
		{
			...
			else if (ri == r_move_input_word)
				new_redirect = make_redirection(sd, r_move_input, rd, 0);
			else if (ri == r_move_output_word)
		...
		/* Set up the variables needed by the rest of the function from the
		   new redirection. */
		if (new->redirect->ins == r_err_and_out)
		{
			char *alloca_hack;
			/* Copy the word without allocating any memory that must be
			   explicitly freed. */
			redirectee = (t_wdtk*)alloca(sizeof(t_wdtk));
			ft_memcpy((char*)new_redirect->redirectee.filename, (char*)redirectee, sizeof(t_wdtk));
			alloca_hack = (char*)alloca(1 + ft_strlen(new_redirect->redirectee.filename->word));
			redirect->word = alloca_hack;
			ft_strcpy(redirectee->word, new_redirect->redirectee.filename->word);
		}
		else
	...
	case r_output_force:
		...
		if (redirectee_word == NULL)
			return (AMBIGUOUS_REDIRECT);
		if (g_restricted && (WRITE_REDIRECT(ri)))
		{
			free(redirectee_word);
			return (RESTRICTED_REDIRECT);
		}
		...
		if (flags & RX_ACTIVE)
		{
			...
			/* If we're remembering, then this is the result of a while, for
			   ...
			   to be close-on-exec to duplicat the effect of old
	...
	case r_reading_string:
		/* REDIRECTEE is a pointer to a T_WDTK containing the text of
	...
	case r_move_output:
		...
		if ((flags & RX_ACTIVE) && (redir_fd != redirector))
		{
			...
			{
				/* Only setup to undo it if the thing to undo is active. */
				if (fcntl(redirector, FD_GETFD, 0) != -1)
			...
			{
				/* r_move_input and r_move output add an additional close()
...
#endif
			/* First duplicate the close-on-exec state of the redirectee. dup2
			   ...
			   always be open. */
			/* if ((already set || set_unconditionally) && (ok_to_set))
			   set_it() */
			...
			if ((redirect->flags & RX_INTERNAL) && (redirect->flags & RX_SAVCLEXEC) && redirector >= 3 &&
...
/* Remember the file descriptor associated with the slot FD,
   ...
   start duplicating. If it's less than SHELL_FD_BASE, we're ok,
...
static int	add_undo_redirect(int fd, enum r_instruction ri, int fdbase)
{
	int		new_fd;
	int		clexec_flag;
	int		savefd_flag;
	...
	sd.dest = new_fd;
	rd.dest = 0;
	closed = make_redirection(sd, r_close_this, rd, 0);
	closer->flags |= RX_INTERNAL;
	dummy_redirect = copy_redirect(closer);
	sd.dest = fd;
	rd.dest = new_fd;
	if (fd == 0)
		new_redirect = make_redirection(sd, r_duplicating_input, rd, 0);
...
static int	add_undo_close_redirect(int fd)
{
	t_redir	*closer;
	t_rdtgt	sd;
...
static void	add_exec_redirect(t_redir *dummy_redirect)
{
...
/* Return non-zero if any of the redirections in REDIRS alter the standard
   input. */
int		stdin_redirects(t_redir *redirs)
{
	t_redir	*rp;
	int		n;
	...
	while (rp)
	{
		if ((rp->rflags & REDIR_VARASSIGN) == 0)
			n += stdin_redirection(rp->ins, rp->redirector.dest);
		rp = rp->next;
	}
	return (n);
}
...
static int	redir_varassign(t_redir *redir, int fd)
{
	t_wdtk	*w;
	t_shvar	*v;
	...
	if (v == NULL || readonly_p(v) || noassign_p(v))
...
static int	redir_varvalue(t_redir *redir)
{
	t_shvar *v;
	char *val, *w;
	intmax_t vmax;
	...
	if (vr && (array_p(v) || assoc_p(v)))
		val = get_array_value(w, 0, (int*)NULL, (t_arrind*)0);
	else
